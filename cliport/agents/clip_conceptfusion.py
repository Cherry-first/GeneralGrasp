...
class ConceptFusionAgent(TransporterAgent):
    ...
    def _build_model(self):
        atten_stream_fcn = 'conceptfusion_pick'   # pick map
        key_stream_fcn = 'conceptfusion_place'     # place map
        query_stream_fcn = 'conceptfusion_kernel'   # crop
    ...
    def trans_forward(self, inp, softmax=True):
        ...
        # pdb.set_trace()
        out = self.transport.forward(inp_img, p0, lang_goal, sim, softmax=softmax)
        return out

    def transport_training_step(self, frame, sim, backprop=True, compute_err=False):
        inp_img = frame['img']
        p0 = frame['p0']
        p1, p1_theta = frame['p1'], frame['p1_theta']
        lang_goal = frame['lang_goal']

        inp = {'inp_img': inp_img, 'p0': p0, 'lang_goal': lang_goal, 'sim': sim}
    ...
    def training_step(self, batch, batch_idx):
        ...
        if isinstance(self.transport, Attention):
            ...
            loss1, err1 = self.transport_training_step(frame, sim2)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(self.val_repeats):
            ...
            if isinstance(self.transport, Attention):
                ...
                l1, err1 = self.transport_training_step(frame, sim2, backprop=False, compute_err=True)
                loss1 += l1
    ...
    def act(self, obs, info, goal=None):  # pylint: disable=unused-argument
        ...
        place_inp = {'inp_img': img, 'p0': p0_pix, 'lang_goal': lang_goal, 'sim': sim2}
