...
class conceptfusion(nn.Module):
    ...
    def similarity_map(self, input_color, input_depth, l1, l2, name):
        ...
        idx = 0
        if 'pyramid' in name:
            target_dep = 5  # if 'google' in name:
        if 'hanoi' in name:
            target_dep = 5
        ...
        for mask_data in masks_original:
            ...
            target_count = np.sum(masked_image <= target_dep)
            ...
            if 'bowl' in name:    
                if target_count > 0.3 * total_count:# 过滤桌面和阴影masks
                    continue
            if pixel_count >= 70:  # 过滤小于 70 像素的掩码
                masks.append(mask_data)
        ...
        # 生成原图
        plt.imshow(out)
        plt.savefig("/home/a/acw799/cliport/cliport/visualization/hanoi_image.png")
        # plt.figure(figsize=(20, 20))
        plt.imshow(img)
        show_anns(masks)
        plt.savefig("cliport/visualization/hanoi_masks.png") 
        ...
        # 生成相似度图
        plt.imshow(sim1)
        plt.savefig("/home/a/acw799/cliport/cliport/visualization/hanoi_sim1.png")
        plt.imshow(sim2)
        plt.savefig("/home/a/acw799/cliport/cliport/visualization/hanoi_sim2.png")
