...
class FiLM(nn.Module):
    ...
    def forward(self, text_embedding, visual_embedding):
        """
        text_embedding: [1, 1024] -> 文本嵌入向量
        visual_embedding: [256, 1280] -> 视觉嵌入向量
        """
        # 计算 Gamma 和 Beta 参数
        gamma = self.gamma_fc(text_embedding)  # [1, 512] -> [1, 512]
        beta = self.beta_fc(text_embedding)    # [1, 512] -> [1, 512]

        # 扩展 text_embedding 以便与 visual_embedding 广播
...
class Unetr(nn.Module):
    ...
    def _build_model(self):
        # 处理最深层的text embedding和visual embedding
    ...
    def extract_patch_features(self, input_image, text_feat, kernel=False):
        ...
        x = self.conv_film(x) # 得到最深层的特征 [1, 64, 16, 16]
        # pdb.set_trace()
